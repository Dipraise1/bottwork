...
const BASE_URL =
  "https://api.telegram.org/bot6400241094:AAGs7768Ggdvxy_8W5qhH2oc9OKUvpqcfTU/";
...
async function handleMessage(update) {
  ...
  if (text === "/start") {
    ...
    await sendMessage(
      chatId,
      "Hey! Welcome. What do you want to do today?",
    ...
    await sendMessage(
      chatId,
      "Available roles for web3 freelancers:",
      JSON.stringify(options)
    );
  } else if (
    [
    ...
    if (
      userData[chatId] &&
      userData[chatId].role &&
      userData[chatId].startingPrice
    ) {
      // Finalize the process
      const { role, startingPrice } = userData[chatId];
      await sendMessage(
        chatId,
        `You have selected the role: ${role} and your starting price is: ${startingPrice}. Thank you for providing the information!`
      );
      delete userData[chatId]; // Clean up user data
    } else {
      await sendMessage(
        chatId,
        "You have not selected a role or provided a starting price yet."
      );
    }
  } else {
    ...
    if (userData[chatId] && userData[chatId].role) {
      ...
      await sendMessage(
        chatId,
        `Your starting price (${text}) has been recorded.`
      );
    } else {
      await sendMessage(
        chatId,
        "Please select a role first before providing your starting price."
      );
    }
  }
}
...
async function main() {
  ...
  while (true) {
    const response = await fetch(
      BASE_URL + "getUpdates?offset=" + (offset ? offset : "")
    );
